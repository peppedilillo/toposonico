...
def get_connection() -> sqlite3.Connection:
    """Get a read-only connection to the merged database."""
    if not DATABASE_PATH.exists():
        raise FileNotFoundError(
            f"Merged database not found: {DATABASE_PATH}\n"
...
def main():
    parser = argparse.ArgumentParser(description="Extract training data for autoencoder")
    parser.add_argument(
        "-p", "--popularity",
        type=int,
        default=80,
    ...
    df = pd.read_sql_query(QUERY_GENRES if args.genres else QUERY, conn, params=[args.popularity])
    conn.close()
    ...
    output_path = output_dir / (f"training_pop{args.popularity}_genres.parquet" if args.genres else f"training_pop{args.popularity}.parquet")
