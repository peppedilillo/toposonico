...
def _pivot_knn(key_vals, neighbor_mat, score_mat, knn_k):
    ...
    ranks_f          = (np.arange(len(keys_f), dtype=np.int32)
                        - run_start_per_el.astype(np.int32))
...
def _insert_knn(conn, table, pk_col, neighbor_col, keys_f, ranks_f, neighbors_f, scores_f):
    conn.executemany(
...
def build_albums(conn, lookup_dir, geo_dir, artist_geo):
    ...
    df = (
        lookup
        .merge(album_geo, on="album_rowid", how="inner")
        .merge(
            artist_geo.rename(columns={"lon": "artist_lon", "lat": "artist_lat"}),
            on="artist_rowid", how="left",
        )
    )
    df = df[["album_rowid", "album_name", "track_count", "logcounts", "lon", "lat",
...
def build_tracks(conn, lookup_dir, geo_dir, artist_geo, album_geo, label_df, batch_size):
    ...
    label_info   = (
        label_df[["label_id", "label", "lon", "lat"]]
        .rename(columns={"lon": "label_lon", "lat": "label_lat"})
    )

    lookup_cols = [
        "track_rowid", "track_name", "track_popularity", "logcounts",
        "release_date", "artist_rowid", "artist_name", "album_rowid", "album_name", "label",
    ]
    ...
    for batch in pf.iter_batches(batch_size=batch_size, columns=lookup_cols):
        ...
        df[[
            "track_rowid", "track_name", "track_popularity", "logcounts", "release_date",
            "lon", "lat",
            "artist_rowid", "artist_name", "artist_lon", "artist_lat",
            "album_rowid",  "album_name",  "album_lon",  "album_lat",
            "label_id", "label", "label_lon", "label_lat",
...
def build_knn_tables(conn, knn_dir, label_to_id, knn_k, batch_size):
    ...
    for entity, pk_col, table in [
        ...
        for knn_b, score_b in zip(
            ...
            _insert_knn(conn, table, pk_col, "neighbor_rowid",
                        keys_f, ranks_f, neighbors_f, scores_f)
    ...
    for knn_b, score_b in zip(
        ...
        _insert_knn(conn, "label_knn", "label_id", "neighbor_id",
                    keys_f, ranks_f, neighbors_f, scores_f)
...
def main():
    ...
    for name, val in [("--lookup-dir", args.lookup_dir),
    ...
    build_tracks(conn, lookup_dir, geo_dir, artist_geo, album_geo, label_df, args.batch_size)
    conn.commit()
